	...
	FormList.prototype = {
		render: function(){
			...
			function createHtml(obj, groupKey){
			    var nodes = "", 
			    	row = '<div class="form-row"><label class="col-xs-6 form-title">{key}</label><div class="col-xs-6 form-text" data-key="{key}">{value}</div></div>';
			    ...
			    function addNode(titleItem, dataItems) {
                    ...
                    for(var j = 0, jl = titleItem.items.length; j < jl; j++){
                        ...
                        nodes +=row.replace(/{key}/g, target.title).replace(/{value}/g, dataItems[target.field]);
                    }
                    return nodes;
                }
